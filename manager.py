...
from .api import Meme, MemeInfo, get_memes, search_memes
...
class MemeConfig:
    def __init__(self, mode: MemeMode = MemeMode.BLACK, white_list: list[str] = None, black_list: list[str] = None):
        self.mode = mode
...
class MemeManager:
    ...
    async def init(self, disabled_list: list[str] = None):
        ...
        self.__meme_dict = {
            meme.key: meme
            for meme in filter(
                lambda meme: meme.key not in disabled_list,
                memes,
            )
        }
        self.__load()
    ...
    def __load(self):
        raw_list: dict[str, dict] = {}
        if self.__path.exists():
            with open(self.__path, "r", encoding="utf-8") as f:
                try:
                    raw_list = yaml.safe_load(f) or {}
                except Exception:
                    logger.warning("表情管理配置加载失败，将使用默认配置")
        try:
            meme_list = {
                name: MemeConfig.from_dict(config)
                for name, config in raw_list.items()
            }
        except Exception:
            meme_list = {}
            logger.warning("表情管理配置解析失败，将使用默认配置")

        self.__meme_config = {
            meme_key: MemeConfig()
            for meme_key in self.__meme_dict.keys()
        }
    ...
    def __dump(self):
        ...
        meme_list = {
            name: config.to_dict()
            for name, config in self.__meme_config.items()
        }
